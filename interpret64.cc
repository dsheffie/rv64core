...
static void execSpecial2(uint32_t inst, state64_t *s);
static void execSpecial3(uint32_t inst, state64_t *s);
static void execCoproc0(uint32_t inst, state64_t *s);
static void execCoproc2(uint32_t inst, state64_t *s);

template <bool EL> void execMips(state64_t *s);

static inline int64_t u32toi64(uint32_t u) {
  return static_cast<int64_t>(*reinterpret_cast<int32_t*>(&u));
}
...
static uint32_t getConditionCode(state64_t *s, uint32_t cc);
static void setConditionCode(state64_t *s, uint32_t v, uint32_t cc);


/* IType instructions */
static void _lb(uint32_t inst, state64_t *s);
static void _lbu(uint32_t inst, state64_t *s);
static void _sb(uint32_t inst, state64_t *s);


static void _mtc1(uint32_t inst, state64_t *s);
static void _mfc1(uint32_t inst, state64_t *s);

static void _sc(uint32_t inst, state64_t *s);

/* FLOATING-POINT */
static void _c(uint32_t inst, state64_t *s);

static void _cvts(uint32_t inst, state64_t *s);
static void _cvtd(uint32_t inst, state64_t *s);

static void _truncw(uint32_t inst, state64_t *s);
static void _truncl(uint32_t inst, state64_t *s);

static void _movci(uint32_t inst, state64_t *s);

static void _fmovc(uint32_t inst, state64_t *s);
static void _fmovn(uint32_t inst, state64_t *s);
static void _fmovz(uint32_t inst, state64_t *s);


static void _movcs(uint32_t inst, state64_t *s);
static void _movcd(uint32_t inst, state64_t *s);

static void _movnd(uint32_t inst, state64_t *s);
static void _movns(uint32_t inst, state64_t *s);
static void _movzd(uint32_t inst, state64_t *s);
static void _movzs(uint32_t inst, state64_t *s);
...
static void execSpecial2(uint32_t inst,state64_t *s) {
  ...
  switch(funct)
    ...
    case(0x20): /* clz */
      ...
      break;
    default:
      printf("unhandled special2 instruction @ 0x%lx\n", s->pc); 
      exit(-1);
      break;
    }
  s->pc += 4;
...
static void execSpecial3(uint32_t inst,state64_t *s) {
  ...
  if(funct == 32) {
	...
	break;
      default:
	printf("unhandled special3 instruction @ 0x%lx\n", s->pc); 
	exit(-1);    
	break;
      }
  }
...
template <typename T>
struct c1xExec {
  void operator()(const coproc1x_t& insn, state64_t *s) {
...
void lxc1(uint32_t inst, state64_t *s) {
  mips_t mi(inst);
...
template <bool EL>
static void execCoproc1x(uint32_t inst, state64_t *s) {
  mips_t mi(inst);
...
static void branch(uint32_t inst, state64_t *s) {
  uint32_t rt = (inst >> 16) & 31;
  uint32_t rs = (inst >> 21) & 31;
...
template <bool EL>
static void _bgez_bltz(uint32_t inst, state64_t *s) {
  uint32_t rt = (inst >> 16) & 31;
  switch(rt&3)
    {
    case 0:
...
static inline
int64_t generateEA(uint32_t inst, state64_t *s) {
  uint32_t rs = (inst >> 21) & 31;
  int16_t himm = static_cast<int16_t>(inst & ((1<<16) - 1));
  int64_t imm = static_cast<int64_t>(himm);
  return s->gpr[rs] + imm;
}

template <bool EL>
static void _lw(uint32_t inst, state64_t *s) { /* mips64 */
  uint32_t rt = (inst >> 16) & 31;
  s->gpr[rt] = bswap<EL>(*((int32_t*)(s->mem + generateEA(inst, s))));
  s->pc += 4;
}

template <bool EL>
static void _lh(uint32_t inst, state64_t *s) { /* mips64 */
  uint32_t rt = (inst >> 16) & 31;
  int16_t mem = bswap<EL>(*((int16_t*)(s->mem + generateEA(inst, s))));
  s->gpr[rt] = static_cast<int64_t>(mem);
  s->pc +=4;
}


static void _lb(uint32_t inst, state64_t *s){ /* mips64 */
  uint32_t rt = (inst >> 16) & 31;
  int8_t v = *((int8_t*)(s->mem + generateEA(inst, s)));
  s->gpr[rt] = static_cast<int64_t>(v);
  s->pc += 4;
}
...
template <bool EL>
static void _lhu(uint32_t inst, state64_t *s) {
  uint32_t rt = (inst >> 16) & 31;
  uint32_t rs = (inst >> 21) & 31;
  int16_t himm = (int16_t)(inst & ((1<<16) - 1));
  int32_t imm = (int32_t)himm;
...
template <bool EL>
static void _sw(uint32_t inst, state64_t *s) { /* mips64 */
  uint32_t rt = (inst >> 16) & 31;
  *((int32_t*)(s->mem + generateEA(inst,s))) = bswap<EL>(s->gpr[rt]);
  s->pc += 4;
}
...
template <bool EL>
static void _sh(uint32_t inst, state64_t *s) {
  uint32_t rt = (inst >> 16) & 31;
  uint32_t rs = (inst >> 21) & 31;
  int16_t himm = (int16_t)(inst & ((1<<16) - 1));
  int32_t imm = (int32_t)himm;
...
template <bool EL>
void _swl(uint32_t inst, state64_t *s) {
  uint32_t rt = (inst >> 16) & 31;
  uint32_t rs = (inst >> 21) & 31;
  int16_t himm = (int16_t)(inst & ((1<<16) - 1));
  int32_t imm = (int32_t)himm;
...
template <bool EL>
void _swr(uint32_t inst, state64_t *s) {
  uint32_t rt = (inst >> 16) & 31;
  uint32_t rs = (inst >> 21) & 31;
  int16_t himm = (int16_t)(inst & ((1<<16) - 1));
  int32_t imm = (int32_t)himm;
  ...
  xx = (x << xs) | (rm & r);
  // std::cout << "SIM SWR EA " << std::hex << ea
  // 	    << ", MA = " << ma
  // 	    << ", X = " << x
  // 	    << ", R = " << r
  // 	    << ", XX = " << xx << std::dec << "\n";	  
...
template <bool EL>
void _lwl(uint32_t inst, state64_t *s) {
  uint32_t rt = (inst >> 16) & 31;
  uint32_t rs = (inst >> 21) & 31;
  int16_t himm = (int16_t)(inst & ((1<<16) - 1));
  int32_t imm = (int32_t)himm;
...
template<bool EL>
void _lwr(uint32_t inst, state64_t *s) {
  uint32_t rt = (inst >> 16) & 31;
  uint32_t rs = (inst >> 21) & 31;
  int16_t himm = (int16_t)(inst & ((1<<16) - 1));
  int32_t imm = (int32_t)himm;
...
template <bool EL>
void _monitorBody(uint32_t inst, state64_t *s) {
...
template <bool EL>
static void _ldc1(uint32_t inst, state64_t *s) {
...
template <bool EL>
static void _sdc1(uint32_t inst, state64_t *s) {
  uint32_t ft = (inst >> 16) & 31;
  uint32_t rs = (inst >> 21) & 31;
  int16_t himm = (int16_t)(inst & ((1<<16) - 1));
  int32_t imm = (int32_t)himm;
...
template <bool EL>
static void _lwc1(uint32_t inst, state64_t *s) {
  uint32_t ft = (inst >> 16) & 31;
  uint32_t rs = (inst >> 21) & 31;
  int16_t himm = (int16_t)(inst & ((1<<16) - 1));
  int32_t imm = (int32_t)himm;
...
template <bool EL>
static void _swc1(uint32_t inst, state64_t *s) {
  uint32_t ft = (inst >> 16) & 31;
  uint32_t rs = (inst >> 21) & 31;
  int16_t himm = (int16_t)(inst & ((1<<16) - 1));
  int32_t imm = (int32_t)himm;
...
template <typename T>
static void fpCmp(uint32_t inst, state64_t *s) {
...
void execFP(uint32_t inst, state64_t *s) {
...
void do_fp_op(uint32_t inst, state64_t *s) {
  switch((inst>>21)&31) {
  case FMT_S:
...
template <bool EL>
void execCoproc1(uint32_t inst, state64_t *s) {
  ...
  if(fmt == 0x8)
	...
	switch(lowop)
	  ...
	  case 0x21:
	    ...
	    break;
	  default:
	    printf("unhandled coproc1 instruction (%x) @ %lx\n",
		   inst, s->pc);
	    exit(-1);
	    break;
	  }
...
void execMips(state64_t *s) {
  ...
  if(isRType) {
	...
	*((uint32_t*)&(s->hi)) = (uint32_t)(y>>32);
	...
	s->pc += 4;
	break;
      case 0x21:  /* addu - mips64 */
	s->gpr[rd] = u32toi64(*reinterpret_cast<uint32_t*>(&s->gpr[rs]) + *reinterpret_cast<uint32_t*>(&s->gpr[rt]));
	s->pc += 4;
	break;
      case 0x22: /* sub */
	printf("sub()\n");
	exit(-1);
	break;
      case 0x23:{ /*subu - mips64*/
	s->gpr[rd] = u32toi64(*reinterpret_cast<uint32_t*>(&s->gpr[rs]) - *reinterpret_cast<uint32_t*>(&s->gpr[rt]));
	s->pc += 4;
	break;
      }
      case 0x24: /* and - mips64*/
	s->gpr[rd] = s->gpr[rs] & s->gpr[rt];
	s->pc += 4;
	break;
      case 0x25: /* or - mips64 */
	...
	s->pc += 4;
	break;
      case 0x26: /* xor - mips64 */
	s->gpr[rd] = s->gpr[rs] ^ s->gpr[rt];
	s->pc += 4;
	break;
      case 0x27: /* nor - mips64 */
	s->gpr[rd] = ~(s->gpr[rs] | s->gpr[rt]);
	s->pc += 4;
	break;
      case 0x2A: /* slt */
	...
	break;
      default:
	printf("%sunknown RType instruction %lx, funct = %d%s\n", 
	...
	break;
      default:
	printf("unknown %s instruction @ %lx", __func__, s->pc); exit(-1);
	break;
      }
    s->pc += 4;
  }
    ...
    uint64_t uimm64 = uimm32;
    int16_t simm16 = static_cast<int16_t>(uimm32);
    int32_t simm32 = static_cast<int32_t>(simm16);
    int32_t simm64 = static_cast<int64_t>(simm32);
    switch(opcode) 
      {
      case 0x01:
	...
	break;
      case 0x08: /* addi - mips64 */
	s->gpr[rt] = s->gpr[rs] + simm64;  
	s->pc+=4;
	break;
      case 0x09: /* addiu - mips64 */
	s->gpr[rt] = s->gpr[rs] + simm64;  
	s->pc+=4;
	break;
      case 0x0A: /* slti - mips64 */
	s->gpr[rt] = (s->gpr[rs] < simm64);
	s->pc += 4;
	break;
      case 0x0B:/* sltiu - mips64 */
	s->gpr[rt] = static_cast<uint64_t>(s->gpr[rs]) < static_cast<uint64_t>(simm64);
	s->pc += 4;
	break;
      case 0x0c: /* andi - mips64 */
	s->gpr[rt] = s->gpr[rs] & uimm64;
	s->pc += 4;
	break;
      case 0x0d: /* ori - mips64 */
	s->gpr[rt] = s->gpr[rs] | uimm64;
	s->pc += 4;
	break;
      case 0x0e: /* xori - mips64 */
	s->gpr[rt] = s->gpr[rs] ^ uimm64;
	s->pc += 4;
	break;
      case 0x0F: /* lui - mips64 */
	uimm32 <<= 16;
	s->gpr[rt] = u32toi64(uimm32);
	s->pc += 4;
	break;
      case 0x14:
	...
	break;
      default:
	printf("%s: Unknown IType instruction (bits=%x) @ pc=0x%lx\n", 
